...
<?php if ($usertype != 'student') : ?>
    <button class="btn btn-primary" data-toggle="modal" data-target="#newexternal"><i class='far fa-calendar-plus'></i> New</button>
    <br>
<?php endif ?>
<br>
<?php if ($externals) : ?>
        ...
        <div class="row">
            <div class="col-md-5">
                <div class="card">
                    <div class="card-body">
                        <h4><?= $ext['title'] ?></h4>
                        <span class="badge rounded-pill bg-dark text-white"><?= $ext['academicsession']['academicsession'] ?></span>&nbsp;<span class="badge rounded-pill bg-warning"><?= $ext['level'] ?></span><small class="post-date">Endorsed
                            by:
                            <?= $ext['mentorname'] ?></small>
                        <p><?= sprintf('%s: %s', '<b>' . date_format(date_create($ext['date']), 'M d, Y') . '</b>', $ext['description']) ?></p>
                        <?php if ($usertype != 'student') : ?>
                            <a data-toggle="modal" data-target="#editexternal" data-id="<?= $ext['id'] ?>" data-title="<?= $ext['title'] ?>" data-description="<?= $ext['description'] ?>" data-date="<?= $ext['date'] ?>" class="btn btn-dark btn-sm" href=""><i data-toggle="tooltip" title="Edit Activity" class='fas fa-pen'></i></a>
                            <a data-toggle="modal" data-target="#addparticipant" data-externalid="<?= $ext['id'] ?>" data-availableid="<?= $i ?>" data-title="<?= $ext['title'] ?>" class="btn btn-dark btn-sm" href=""><i data-toggle="tooltip" title="Add Participant" class='fas fa-user-plus'></i></a>
                            <a data-toggle="modal" data-target="#deleteexternal" data-externalid="<?= $ext['id'] ?>" data-externaltitle="<?= $ext['title'] ?>" class="btn btn-dark btn-sm" href=""><i data-toggle="tooltip" title="Delete Activity" class='fa fa-trash'></i></a>
                        <?php endif ?>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col">
                        ...
                        <?php if ($ext['participants']) : ?>
                            <?php foreach ($ext['participants'] as $participant) : ?>
                                <div class="img-wrap">
                                    <?php if ($usertype != 'student') : ?>
                                        <a data-toggle="modal" data-target="#deleteparticipant" data-title="<?= $ext['title'] ?>" data-name="<?= $participant['name'] ?>" data-userid="<?= $participant['id'] ?>" data-externalid="<?= $ext['id'] ?>" class="close">&times;</a>
                                    <?php endif ?>
                                    <a data-name="<?= $participant['name'] ?>" href="<?= site_url('student/' . $participant['id']) ?>"><img data-toggle="tooltip" title="<?= $participant['name'] ?>" class="rounded-circle" style="object-fit:cover;" src="<?= $participant['userphoto'] ?>" alt="<?= $participant['name'] ?>" width="60px" height="60px"></a>
                                </div>
                            <?php endforeach ?>
                        <?php endif ?>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <div id="externalbtndiv">
                        <button id="btnaddexternal" class="btn btn-dark" type="submit">Submit</button>
                    </div>
